...
<div class="offcanvas offcanvas-bottom" data-bs-keyboard="false" tabindex="-1" data-bs-backdrop="true" id="off-canvas-all-menu" aria-labelledby="offcanvasBottomLabel">
    ...
    <div class="offcanvas-body text-center small scrollable-menu">
        <ul class="d-flex flex-wrap justify-content-center list-unstyled">
            ...
            @endphp
            @foreach($all_menus as $role_menus)
            @if($role_menus->Menu == NULL)
            @else
            @php
            // 1. Pilih kunci (index) acak dari array $color
            $randomKey = array_rand($color);

            // 2. Ambil nilai warna menggunakan kunci acak tersebut
            $randomColorClass = $color[$randomKey];
            @endphp
            <li class="text-center">
                <a id="btn_klik" class="nav-link " href="{{ url($role_menus->Menu->url) }}">
                    <span class="dz-icon {{$randomColorClass}} light" style="height: 50px; width: 50px; box-shadow: 0 1px 1px 0 rgba(0, 0, 0, 0.2), 0 3px 10px 0 rgba(0, 0, 0, 0.1);">
                        {!!$role_menus->Menu->icon!!}
                    </span>
                </a>
                <span>{{ $role_menus->Menu->name }}</span>
            </li>
