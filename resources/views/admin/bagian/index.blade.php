                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <button type="button" class="btn btn-sm btn-primary waves-effect waves-light"
                            data-bs-toggle="modal" data-bs-target="#modal_tambah_bagian"><i
                                class="menu-icon tf-icons mdi mdi-plus"></i>Tambah</button>
                        <button type="button" class="btn btn-sm btn-success waves-effect waves-light"
                            data-bs-toggle="modal" data-bs-target="#modal_import_bagian"><i
                                class="menu-icon tf-icons mdi mdi-file-excel"></i>Import</button>
                        <div class="modal fade" id="modal_tambah_bagian" data-bs-backdrop="static" tabindex="-1">
                            <div class="modal-dialog modal-dialog-scrollable">
                                <form method="post"
                                    action="@if (Auth::user()->is_admin == 'hrd') {{ url('/hrd/bagian/insert/' . $holding->holding_code) }}@else{{ url('/bagian/insert/' . $holding->holding_code) }} @endif"
                                            ...
                                            <div class="col mb-2">
                                                <div class="form-floating form-floating-outline">
                                                    <select class="form-control @error('nama_divisi') is-invalid @enderror"
                                                        id="form_nama_divisi" name="nama_divisi">
                        ...
                        <div class="modal fade" id="modal_import_bagian" data-bs-backdrop="static" tabindex="-1">
                            <div class="modal-dialog modal-dialog-scrollable modal-lg">
                                <form method="post"
                                    action="@if (Auth::user()->is_admin == 'hrd') {{ url('/hrd/bagian/ImportBagian/' . $holding->holding_code) }}@else{{ url('/bagian/ImportBagian/' . $holding->holding_code) }} @endif"
                                    class="modal-content" enctype="multipart/form-data">
                                    @csrf
                                    <div class="modal-header">
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <div class="row g-2 mt-2">
                                            <a href="{{ asset('') }}" type="button" download=""
                                                class="btn btn-sm btn-primary"> Download Format Excel</a>
                                        </div>
                                    </div>
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">
                        ...
                        <div class="modal fade" id="modal_edit_bagian" data-bs-backdrop="static" tabindex="-1">
                            <div class="modal-dialog modal-dialog-scrollable">
                                <form method="post"
                                    action="@if (Auth::user()->is_admin == 'hrd') {{ url('/hrd/bagian/update/' . $holding->holding_code) }}@else{{ url('/bagian/update/' . $holding->holding_code) }} @endif"
                                    class="modal-content" enctype="multipart/form-data">
                                    @csrf
                                    <div class="modal-header">
                        ...
                        <div class="modal fade" id="modal_lihat_karyawan" data-bs-backdrop="static" tabindex="-1">
                            <div class="modal-dialog modal-lg modal-dialog-scrollable">
                                <div class=" modal-content">
                                    ...
                                    <div class="modal-body">
                                        <div class="col-lg-12">
                                            <table class="table" id="table_lihat_karyawan"
                        ...
                        <div class="modal fade" id="modal_lihat_jabatan" data-bs-backdrop="static" tabindex="-1">
                            <div class="modal-dialog modal-lg modal-dialog-scrollable">
                                <div class=" modal-content">
                                    ...
                                    <div class="modal-body">
                                        <div class="col-lg-12">
                                            <table class="table" id="table_lihat_jabatan"
                                                style="width: 100%; font-size: smaller;">
                                                <thead class="table-primary">
                                                    <tr>
                                                        <th>No.</th>
...
@section('js')
    ...
    <script>
        ...
        var table = $('#table_bagian').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "@if (Auth::user()->is_admin == 'hrd'){{ url('hrd/bagian-datatable') }}@else{{ url('bagian-datatable') }}@endif" +
                    '/' + holding,
            },
            columns: [{
                    data: "id",
    ...
    <script>
        ...
        $(document).on('click', '#btn_delete_bagian', function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: "@if (Auth::user()->is_admin == 'hrd'){{ url('/hrd/bagian/delete/') }}@else{{ url('/bagian/delete/') }}@endif" +
                            '/' + id + '/' + holding,
                        type: "GET",
                        data: {
                            id: id,
        ...
        $(document).on('click', '#btn_lihat_jabatan', function() {
            ...
            let holding = $(this).data("holding");
            let url =
                "@if (Auth::user()->is_admin == 'hrd'){{ url('hrd/jabatan/jabatan-datatable') }}@else{{ url('jabatan/jabatan-datatable') }}@endif" +
                '/' + id + '/' + holding;
            // console.log(url);
        ...
        $(document).on('click', '#btn_lihat_karyawan', function() {
            ...
            let holding = $(this).data("holding");
            let url =
                "@if (Auth::user()->is_admin == 'hrd'){{ url('hrd/jabatan/karyawanjabatan-datatable') }}@else{{ url('jabatan/karyawanjabatan-datatable') }}@endif" +
                '/' + id + '/' + holding;
            console.log(id);
