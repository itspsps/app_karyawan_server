        ...
        <script>
            $(document).ready(function() {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            $.each(data.data_columns_header, function(i, col) {
                                $('#date_absensi').append('<th class="text-center">' + col
                                    .header + '</th>');
                            });
                            ...
                            load_data(departemen_filter, divisi_filter, bagian_filter,
                                jabatan_filter, shift_filter, start_date, end_date, data_column);
                        },
                        error: function(data) {
                ...
                $('#departemen_filter').change(function(e) {
                    ...
                    var url =
                        "@if (Auth::user()->is_admin == 'hrd'){{ url('hrd/report_kedisiplinan/get_divisi') }}@else{{ url('report_kedisiplinan/get_divisi') }}@endif" +
                        '/' + holding_id;
                ...
                $('#divisi_filter').change(function() {
                    ...
                    $.ajax({
                        type: 'GET',
                        url: "@if (Auth::user()->is_admin == 'hrd'){{ url('hrd/report_kedisiplinan/get_bagian') }}@else{{ url('report_kedisiplinan/get_bagian') }}@endif" +
                            '/' + holding_id,
                        data: {
                            holding: holding_id,
                            divisi_filter: divisi_filter,
                ...
                function load_data(departemen_filter = '', divisi_filter = '', bagian_filter = '', jabatan_filter = '',
                    shift_filter = '', start_date = '', end_date = '', data_column = '') {
                    ...
                    var table_rekapdata = $('#table_rekapdata').DataTable({
                        ...
                        buttons: [{
                                ...
                                filename: function() {
                                    ...
                                    return 'REKAP_DATA_ABSENSI_KARYAWAN_{{ $holding->holding_name }}_' +
                                        l + ' ' + n;
                                },
                            },
                            {
                        ...
                        ajax: {
                            url: "@if (Auth::user()->is_admin == 'hrd'){{ url('hrd/report_kedisiplinan-datatable') }}@else {{ url('report_kedisiplinan-datatable') }}@endif" +
                                '/' + holding,
                            data: {
                                start_date: start_date,
                                end_date: end_date,
                ...
                function get_grafik_absensi(departemen_filter = '', divisi_filter = '', bagian_filter = '',
                    jabatan_filter = '', start_date = '', end_date = '') {
                    // console.log(filter_month);
                    $.ajax({
                        url: "@if (Auth::user()->is_admin == 'hrd'){{ url('hrd/report_kedisiplinan/get_grafik_absensi') }}@else{{ url('report_kedisiplinan/get_grafik_absensi') }}@endif" +
                            '/' + holding,
                        data: {
                            holding: holding,
                            start_date: start_date,
                        ...
                        success: function(data) {
                            ...
                            var chart_absensi_masuk = new ApexCharts(document.querySelector(
                                "#grafik_absensi"), options);
