        ...
        if (count($jabatan) == 0) {
            ...
            foreach ($jabatan as $jabatan) {
                ...
                $ok = Karyawan::Join('jabatans', 'jabatans.id', 'karyawans.jabatan_id')
                    ...
                    ->whereIn('karyawans.penempatan_kerja', ['ALL SITES (SP, SPS, SIP)', 'CV. SUMBER PANGAN - KEDIRI', 'CV. SUMBER PANGAN - SUBANG', 'ALL SITES (SP)'])
                ...
                $foto = '<img width=30 height=30 style="border-radius: 50%;" align=center margin_bottom=4 margin_top=4 src=https://hrd.sumberpangan.store:4430/admin/assets/img/avatars/1.png><br>';
        ...
        if (count($jabatan1) == 0) {
            ...
            foreach ($jabatan1 as $jabatan) {
                $ok = Karyawan::Join('jabatans', 'jabatans.id', 'karyawans.jabatan_id')
                    ...
                    ->join('departemens', 'departemens.id', '=', 'karyawans.dept_id')
                    ->join('bagians', 'bagians.id', '=', 'karyawans.bagian_id')
                    ->whereIn('karyawans.penempatan_kerja', ['ALL SITES (SP, SPS, SIP)', 'PT. SURYA PANGAN SEMESTA - KEDIRI', 'PT. SURYA PANGAN SEMESTA - NGAWI', 'PT. SURYA PANGAN SEMESTA - SUBANG', 'ALL SITES (SPS)'])
                    ->where('divisis.nama_divisi', $jabatan["nama_divisi"])
                    ->where('bagians.nama_bagian', $jabatan["nama_bagian"])
                    ->where('jabatans.nama_jabatan', $jabatan["nama_jabatan"])
                    ->where('karyawans.status_aktif', 'AKTIF')
                ...
                $ok2 = Karyawan::Join('jabatans', 'jabatans.id', 'karyawans.jabatan2_id')
                    ...
                    ->join('departemens', 'departemens.id', '=', 'karyawans.dept2_id')
                    ->join('bagians', 'bagians.id', '=', 'karyawans.bagian2_id')
                    ->whereIn('karyawans.penempatan_kerja', ['ALL SITES (SP, SPS, SIP)', 'PT. SURYA PANGAN SEMESTA - NGAWI', 'PT. SURYA PANGAN SEMESTA - SUBANG', 'ALL SITES (SPS)'])
                    ->where('divisis.nama_divisi', $jabatan["nama_divisi"])
                    ->where('bagians.nama_bagian', $jabatan["nama_bagian"])
                    ->where('jabatans.nama_jabatan', $jabatan["nama_jabatan"])
                    ->where('karyawans.status_aktif', 'AKTIF')
    ...
    public function index1()
    {
        ...
        if ($holding == 'sp') {
            ...
            $user = Karyawan::join('jabatans', 'jabatans.id', 'karyawans.jabatan_id')
                ...
                ->where('karyawans.penempatan_kerja', 'CV. SUMBER PANGAN - KEDIRI')
                // ->where('divisis.nama_divisi', 'PRODUCTION')
                ->orWhere('karyawans.penempatan_kerja', 'CV. SUMBER PANGAN - TUBAN')
                ->orWhere('karyawans.penempatan_kerja', 'ALL SITES (SP, SPS, SIP)')
                ->orWhere('karyawans.penempatan_kerja', 'DEPO SPS SIDOARJO')
                // ->where('penempatan_kerja', 'ALL SITES (SP, SPS, SIP)')
                // ->orWhere('karyawans.penempatan_kerja', 'ALL SITES (SP)')
            ...
            $user = Karyawan::join('jabatans', 'jabatans.id', 'karyawans.jabatan_id')
                ...
                ->where('karyawans.kategori', 'Karyawan Bulanan')
                ->whereNotNull('karyawans.bagian_id')
                ->where('karyawans.penempatan_kerja', 'PT. SURYA PANGAN SEMESTA - KEDIRI')
                // ->where('divisis.nama_divisi', 'PRODUCTION')
                ->orWhere('karyawans.penempatan_kerja', 'PT. SURYA PANGAN SEMESTA - NGAWI')
                ->orWhere('karyawans.penempatan_kerja', 'PT. SURYA PANGAN SEMESTA - SUBANG')
                ->orWhere('karyawans.penempatan_kerja', 'ALL SITES (SP, SPS, SIP)')
                ->orWhere('karyawans.penempatan_kerja', 'DEPO SPS SIDOARJO')
                // ->where('penempatan_kerja', 'ALL SITES (SP, SPS, SIP)')
                // ->orWhere('karyawans.penempatan_kerja', 'ALL SITES (SP)')
            ...
            $user = Karyawan::join('jabatans', 'jabatans.id', 'karyawans.jabatan_id')
                ...
                ->where('karyawans.kategori', 'Karyawan Bulanan')
                ->whereNotNull('karyawans.bagian_id')
                ->where('karyawans.penempatan_kerja', 'CV. SURYA INTI PANGAN - MAKASAR')
                ->orWhere('karyawans.penempatan_kerja', 'ALL SITES (SP, SPS, SIP)')
                // ->where('penempatan_kerja', 'ALL SITES (SP, SPS, SIP)')
                // ->orWhere('karyawans.penempatan_kerja', 'ALL SITES (SP)')
                // ->where('departemens.nama_departemen', 'PENGEMBANGAN TEKNOLOGI & SISTEM INFORMASI')
