...
class KaryawanImportUpdate implements ToCollection, WithStartRow
{
    use Importable;
    /**
     * @param array $row
    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $row) {
            ...
            if ($row[9] == NULL || $row[9] == 0) {
                ...
                $tgl_lahir = is_numeric($row[9]) ? Carbon::parse(\PhpOffice\PhpSpreadsheet\Shared\Date::excelToDateTimeObject($row[9]))->format('Y-m-d') : Carbon::createFromFormat('Y-m-d', $row[9])->format('Y-m-d');
            }
            // dd($tgl_lahir);
            // KELAMIN
            ...
            if ($row[28] == NULL || $row[28] == 0) {
                ...
                // $tgl_join = Carbon::parse($row[28])->format('Y-m-d'); 
                $tgl_join = is_numeric($row[28]) ? Carbon::parse(\PhpOffice\PhpSpreadsheet\Shared\Date::excelToDateTimeObject($row[28]))->format('Y-m-d') : Carbon::createFromFormat('Y-m-d', $row[28])->format('Y-m-d');
                // dd($tgl_mulai);
            }
            ...
            if ($row[30] == NULL || $row[30] == '0') {
                ...
                // $tgl_mulai = Carbon::parse($row[30])->format('Y-m-d'); 
                $tgl_mulai = is_numeric($row[30]) ? Carbon::parse(\PhpOffice\PhpSpreadsheet\Shared\Date::excelToDateTimeObject($row[30]))->format('Y-m-d') : Carbon::createFromFormat('Y-m-d', $row[30])->format('Y-m-d');
                // dd($tgl_mulai);
            }
            if ($row[31] == NULL || $row[31] == '0') {
                ...
                $tgl_selesai = is_numeric($row[31]) ? Carbon::parse(\PhpOffice\PhpSpreadsheet\Shared\Date::excelToDateTimeObject($row[31]))->format('Y-m-d') : Carbon::createFromFormat('Y-m-d', $row[31])->format('Y-m-d');
            }
